...
class HourglassPlabicGraph:
    ...
    def get_trip(self, vertex, i, output='half_strands'):
        r"""
        Gets the ``i``th trip starting from the provided vertex. `vertex` should be on the boundary.
    ...
    def get_trip_perm(self, i):
        r"""
        Computes the ``i``th trip permutation.
        The trip ``i`` permutation is generated by finding where trip ``i`` sends each boundary vertex to.
        Boundary vertices are assumed to be numbered ``1``, ``2``, ..., ``n`` clockwise starting slightly east of due north.
    ...
    def get_trip_perms(self):
        r"""
        Computes the trip permutation for each trip ``i``.
    ...
    def separation_labeling(self, base_face, r=4, verbose=False):
        ...
        EXAMPLES:
            ...
            sage: HPG._get_hourglass_by_id(70, 75).label
            [1, 2, 3]
        """
    ...
    def make_circular(self, r=10):
        ...
        .. SEEALSO::
            ...
            :meth:`HourglassPlabicGraph.tutte_layout`
        ...
        .. PLOT::
            ...
            sphinx_plot(HPG)
        """
        n = len(self._boundary_vertices)
        for i, v in self._boundary_vertices.items():
